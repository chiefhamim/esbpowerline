...
export async function getMyNotices() {
  ...
  return prisma.editorialNotice.findMany({
    where: { recipientId: user.id, trashedAt: null },
    include: {
...
export async function getMyPendingNoticeCount() {
  ...
  return prisma.editorialNotice.count({
    where: { recipientId: user.id, status: 'PENDING', trashedAt: null },
  });
}

export async function acknowledgeAllNotices() {
  const user = await requireAuthUser();
  await prisma.editorialNotice.updateMany({
    where: { recipientId: user.id, status: 'PENDING', trashedAt: null },
...
export async function createEditorialNotice(data: {
  ...
  if (!can(role, 'article.review')) throw new Error('Forbidden');
...
export async function requestArticleRevision(articleIds: string[], note: string) {
  const user = await requireAuthUser();
  if (!can(user.role as Role, 'article.review')) throw new Error('Forbidden');

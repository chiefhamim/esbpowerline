...
export type { PublicArticleCard, PublicCategory };
...
export async function getPublishedArticlesForPublic(limit = 60): Promise<PublicArticleCard[]> {
  const articles = await prisma.article.findMany({
    where: { status: 'PUBLISHED' },
    include: { author: { select: { name: true } } },
    orderBy: { publishedAt: 'desc' },
...
export async function getTrendingPublishedArticles(limit = 5): Promise<PublicArticleCard[]> {
  const articles = await prisma.article.findMany({
    where: { status: 'PUBLISHED' },
    include: { author: { select: { name: true } } },
    orderBy: { views: 'desc' },
...
export async function getLatestMagazineIssue(): Promise<PublicMagazineIssue | null> {
